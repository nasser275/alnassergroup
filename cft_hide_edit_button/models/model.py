...
def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
    ...
    if self.env.user.has_group('cft_hide_edit_button.group_hide_edit_button'):
        if self._name != 'res.users' and self._name != 'alfa.replanishment' and self._name != 'return.replanishment':
            if not self.env.ref('cft_hide_edit_button.group_hide_edit_button').sudo().hide_edit_objects:
                temp = etree.fromstring(result['arch'])
                temp.set('edit', '0')
                result['arch'] = etree.tostring(temp)
            else:
                models = self.env.ref('cft_hide_edit_button.group_hide_edit_button').sudo().hide_edit_objects
