...
class returnTransferOrderReport(models.Model):
    _name = "report.return"
    _auto = False
    _order = 'date desc'

    date = fields.Datetime(string='Order Date', readonly=True)
    transfer_id = fields.Many2one('return.replanishment', string='Order', readonly=True)
    ...
    def _from(self):
        return """
            FROM return_replenishment_line AS l
                INNER JOIN return_replanishment s ON (s.id=l.return_replenishment_id)
