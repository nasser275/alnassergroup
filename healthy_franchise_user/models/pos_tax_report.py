...
class PosTaxReport(models.Model):
    _name = "report.pos.tax"
    _auto = False
    _order = 'date desc'
    ...
    total_untaxed = fields.Float("Total Without Tax", readonly=True)
    total_tax = fields.Float("Total Tax", readonlyF=True)
    ...
    def _select(self):
        return """
            SELECT
                ...
                SUM((l.price_subtotal_incl - l.price_subtotal) / CASE COALESCE(s.currency_rate, 0) WHEN 0 THEN 1.0 ELSE s.currency_rate END) AS total_tax,
                s.id as order_id,
