...
class PosOrder(models.Model):
    ...
    discount_by = fields.Selection(string="Discount By", selection=[('offer', 'Offer'), ('code', 'Code'), ],
                                   required=False)
    discount_code = fields.Char(string="Application Code")
    discount_reason_id = fields.Many2one(comodel_name="pos.discount.reasons", string="Pos Application Reason",
                                         required=False)
    discount_offer_id = fields.Many2one(comodel_name="pos.discount.reasons", string="Pos Discount Offer",
                                        required=False)
    call_center_agent_name = fields.Char(string='Call Center Agent')

    def _prepare_refund_values(self, current_session):
        data = super(PosOrder, self)._prepare_refund_values(current_session)
        data['order_source_id'] = self.order_source_id.id
        data['discount_type'] = self.discount_type
        data['discount_by'] = self.discount_by
        data['discount_code'] = self.discount_code
        data['discount_reason_id'] = self.discount_reason_id.id
        data['discount_offer_id'] = self.discount_offer_id.id
        data['sales_person'] = self.sales_person.id
        return data

    def _export_for_ui(self, order):
        data = super(PosOrder, self)._export_for_ui(order)
        data['order_source_id'] = order.order_source_id.id
        data['discount_type'] = order.discount_type
        data['discount_by'] = order.discount_by
        data['discount_code'] = order.discount_code
        data['discount_reason_id'] = order.discount_reason_id.id
        data['discount_offer_id'] = order.discount_offer_id.id
        data['sales_person'] = order.sales_person.id
        return data

    def _order_fields(self, ui_order):
        order = super(PosOrder, self)._order_fields(ui_order)
        order['order_source_id'] = ui_order.get('order_source_id')
        order['discount_type'] = ui_order.get('discount_type')
        order['discount_by'] = ui_order.get('discount_by')
        order['discount_code'] = ui_order.get('discount_code')
        order['discount_reason_id'] = ui_order.get('discount_reason_id')
        order['discount_offer_id'] = ui_order.get('discount_offer_id')
        return order


class PosOrderSource(models.Model):
    _name = 'pos.order.source'
    name = fields.Char(string="Name", required=True)
    required_discount_reason = fields.Boolean(string="Require Discount Application")
    active = fields.Boolean(string='Active', default=True)
    remove_promotion = fields.Boolean(string="Remove Promotion")
    show = fields.Boolean(string="Is Show In POS", default=True)


class PosConfig(models.Model):
    _inherit = 'pos.config'
    show_order_source = fields.Boolean(string='Show Order Source', default=False)
    enable_discount = fields.Boolean(string="Enable Applications Discount")
    enable_fdiscount = fields.Boolean(string="Enable Fixed Discount")
    enable_pdiscount = fields.Boolean(string="Enable Percentage Discount")
    fixed_min_order_total = fields.Float(string='Fixed -Min Order Total')
    fixed_max_order_total = fields.Float(string='Fixed -Max Discount')


    healthy_discount_product_id = fields.Many2one('product.product', string='Applications Discount Product',
                                                  domain="[('sale_ok', '=', True)]",
                                                  help='The product used to model the discount.')
    healthy_fdiscount_product_id = fields.Many2one('product.product', string='Fixed Discount Product',
                                                   domain="[('sale_ok', '=', True)]",
                                                   help='The product used to model the discount.')
    healthy_pdiscount_product_id = fields.Many2one('product.product', string='Percentage Discount Product',
                                                   domain="[('sale_ok', '=', True)]",
                                                   help='The product used to model the discount.')

    perc_min_order_total = fields.Float(string='Percentage -Min Order Total')
    perc_max_order_total = fields.Float(string='Percentage -Max Discount')
    customer_required = fields.Boolean(
        string='Customer Required',
        required=False)


class PosReason(models.Model):
    _name = 'pos.discount.reasons'
    name = fields.Char(string="Name", required=True)
    show = fields.Boolean(string="Show In Pos", default=True)
...
class UnitCostLine(models.Model):
    _inherit = "pos.order.line"

    unit_cost = fields.Float(string='Unit Cost', compute='_compute_unit_cost')

    @api.onchange('total_cost', 'qty')
    def _compute_unit_cost(self):
        for line in self:
            if line.qty > 0:
                line.unit_cost = line.total_cost / line.qty
            elif line.qty == 0:
                line.unit_cost = 0
            else:
                line.unit_cost = line.total_cost / line.qty
...
class PosConfig(models.Model):
    ...
    def action_pos_session_closing_control(self, balancing_account=False, amount_to_balance=0,
        ...
        for session in self:
            ...
            if any(order.state == 'in_prgress' for order in session.order_ids):
                raise UserError(_("You cannot close the POS when orders are still in In Progress"))
    ...
    def _cannot_close_session(self, bank_payment_method_diffs=None):
        ...
        if any(order.state == 'draft' for order in self.order_ids):
            return {'successful': False, 'message': _("You cannot close the POS when orders are still in draft"),
                    'redirect': False}
        if any(order.state == 'in_prgress' for order in self.order_ids):
            return {'successful': False, 'message': _("You cannot close the POS when orders are still in progress"),
                    'redirect': False}
    ...
    def _accumulate_amounts(self, data):
        ...
        for key, value in sales.items():
            if value['amount'] > 0:
                ...
                debit += -1 * value['amount']

        for key, value in combine_receivables_bank.items():
            if value['amount'] > 0:
                debit += value['amount']
            else:
                credit += -1 * value['amount']

        for key, value in combine_receivables_cash.items():
            if value['amount'] > 0:
                debit += value['amount']
            else:
                credit += -1 * value['amount']
        ...
        if diff != 0:
            if diff > -0.02 and diff <= 0.02:
                for key, value in sales.items():
                    ...
                    newamount = value['amount'] + diff
                    newamount_converted = value['amount'] + diff
                    value['amount'] = newamount
                    value['amount_converted'] = newamount_converted
                    break
        """Fix session when closed"""
