odoo.define('app_list.AppOrderFetcher', function (require) {
    'use strict';
    ...
    class AppOrderFetcher extends EventBus {
        constructor() {
            super();
            this.currentPage = 1;
            this.ordersToShow = [];
        ...
        async _getOrderIdsForCurrentPage(limit, offset) {
            return await this.rpc({
                model: 'app.order',
                method: 'search_read',
                args: [this.searchDomain ? this.searchDomain : [], ['name', 'app_lines','discount_type', 'discount', 'customer_notes', 'order_source_id', 'schedule_time', 'delivery_cost', 'amount_total', 'partner_id', 'branch_id', 'final_total', 'order_date', 'state', 'sales_person', 'is_paid'], offset, limit],
