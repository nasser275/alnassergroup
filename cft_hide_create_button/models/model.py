...
def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
    ...
    if self.env.user.has_group('cft_hide_create_button.group_hide_create_button'):
        if self._name != 'res.users':
            if not self.env.ref('cft_hide_create_button.group_hide_create_button').sudo().hide_create_objects:
                temp = etree.fromstring(result['arch'])
                temp.set('create','0')
                result['arch'] = etree.tostring(temp)
            else:
                models = self.env.ref('cft_hide_create_button.group_hide_create_button').sudo().hide_create_objects
